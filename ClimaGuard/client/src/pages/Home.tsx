...
//       try {
//         const apiKey = import.meta.env.VITE_OPENWEATHER_KEY as string; // your key from .env
//         const city = "London"; // can make this dynamic later
...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    const fetchWeather = async () => {
      try {
        const apiKey = import.meta.env.VITE_WEATHER_API as string; // your key from .env
        const city = "London"; // can make this dynamic later
        const res = await fetch(
          `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&appid=${apiKey}`
        );
